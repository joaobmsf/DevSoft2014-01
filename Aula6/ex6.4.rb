...
# Seu código aqui...
puts "Existem #{str.scan(/(^)/).size()} linhas."

# 3) Quantas notícias de economia de falam sobre o dólar?

# Seu código aqui...
puts "#{str.scan(/(^.* dólar.*)/).size()} notícias falam sobre o dólar."

# 4) Imprimir todos os horários das notícias que contém a palavra golfe.

puts "Horarios das notícias que contém a palavra golfe:"
puts str.scan(/^(\d\d:\d\d).* golfe.*/)
...
puts "Temas em com letras em maiúculo:"
dados.each do |m|
  puts m[0]+' '+m[1].upcase+' '+m[2]
end
